...
(function attachArenaRender(arena) {
    ...
    const SPECIAL_TYPE_RULES = Object.freeze([
        ...
        { type: 'FOSSIL', text: 'A Fossil in the knockout pile can revive once, returning with 60% max HP and Fatigue. It never comes back on the turn it went down: from the next turn on it takes the first open board slot at end of turn, ahead of drawing a replacement. If no other Pokemon are left on the board or in the Pokemon deck, it revives at the end of the turn it was knocked out instead. Several waiting Fossils revive in the order they were knocked out. A revived Fossil\'s earlier knockout no longer counts toward the knockout limit.' }
    ]);
    ...
    function renderStatus(pendingAction) {
        ...
        const canDiscardHand = arena.Controller.canDiscardPlayerHand();
        ...
        return `
            <section class="arena-status" aria-label="Turn controls">
                ...
                <div class="action-bar">
                    ${renderActionButtons(canEnd, canDiscard, canDiscardHand, canUndo)}
                </div>
    ...
    function renderActionButtons(canEnd, canDiscard, canDiscardHand, canUndo) {
        ...
        if (['selecting-attack-user', 'selecting-attack-target', 'selecting-item-target'].includes(state.phase)) {
            return `
                ...
                <button class="arena-button arena-button--discard-hand" type="button" disabled>Discard Hand</button>
                ${rulesButton}
                ${menuButton}
            `;
        }
        ...
        return `
            <button class="arena-button arena-button--undo" type="button" data-action="undo" ${canUndo ? '' : 'disabled'}>Undo</button>
            <button class="arena-button arena-button--discard" type="button" data-action="discard-selected" ${canDiscard ? '' : 'disabled'}>Discard</button>
            <button class="arena-button arena-button--discard-hand" type="button" data-action="discard-hand" ${canDiscardHand ? '' : 'disabled'}>Discard Hand</button>
