...
- [x] 1. **`dev/editor/index.html`** — the app page per the overview's stack: `<body
  ...
  id="editor-view">`.
- [x] 2. **`dev/editor/editor.css`** — editor chrome on top of the game theme: tab bar
...
- [x] 3. **`dev/editor/preview.js`** — local normalizers (pokemon/attack/item), the three
...
- [x] 4. **`dev/editor/list_view.js`** — the generic component per the context spec.
  Expose as `window.EditorListView`.
- [x] 5. **`dev/editor/app.js`** — `window.EditorApp`: `init()` (parallel fetch of
...
- [x] 6. **`dev/editor/tab_pokemon.js` … `tab_locations.js`** (6 files) — each registers its
...
- [x] 7. **`dev/editor/tab_issues.js`** — read-only issues list per the context (grouped by
  file, severity filter), sourced from `EditorApp.computeIssues()`.

## Verification

- [x] `node tests/run_all.js` green (all new files syntax-check; no behavior tests yet —
  the UI phases verify in-browser).
- [x] With `node dev/editor/server.js` running, `http://127.0.0.1:8932/` in a browser (or
...
- [x] Trainers tab: 95 rows, sprite thumbnails render (Gamer shows a sprite image);
  Locations tab: 12 rows with theme swatches and type icons.
- [x] Issues badge is visible with a warning count ≥ 13 (12 missing backgrounds + Linoone
  orphan) and 0 errors; the Issues tab lists them grouped by file.
- [x] Visual check: a rendered element with class `playing-card` appears nowhere yet (lists
