...
- [x] 1. **`achievements.html`** (new, repo root) — the `overview.html` skeleton with
...
- [x] 2. **`map/achievements.js`** (new) — the page controller, following
...
- [x] 4. **`map/profile.js`** — add `showPendingUnlocks(achievements)`:
...
- [x] 5. **`static/styles.css`** — add an `.achievements-page` block:
...
- [x] 6. **Six page controllers** — add the one-line drain to each page's init, right after
...
- [x] 7. **`tests/achievements.test.js`** — add a case that `showPendingUnlocks` returns
...
- [x] 9. **`dev/verify/phase95_achievements.py`** — new Playwright driver modeled on the
...
- [x] `dev/verify/phase95_achievements.py` runs clean and its screenshot shows an unlocked
  row, a locked row with a progress bar, and a hidden `???` row.
- [x] Served manually (`python3 -m http.server 8931 --bind 127.0.0.1`): the main menu shows
...
- [x] Toast end to end: clear the profile
...
- [x] A hidden achievement reveals its real name and description once unlocked.
- [x] Responsive check at 390px: rows stack one per line and the toast does not overflow
  the viewport.
