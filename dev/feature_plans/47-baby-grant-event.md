...
- [x] 1. **`events.json`** — add a gift event: id `nursery-egg`, title
...
- [x] 2. **`map/event_effects.js`** — in `chooseEvent`, after collecting available
...
- [x] 3. **`map/event.js`** — `describeEffect`: add
  `if (effect.type === 'gain-random-baby') return 'Gain a random baby Pokemon';`.
- [x] 4. **`tests/data_validation.test.js`** and **`dev/editor/validate.js`** — add
...
- [x] 5. **`tests/baby_event.test.js`** (new, via `tests/helpers/arena_env.js`; require
...
- [x] `node tests/run_all.js` green — this is the proof the event is inert against live
  data with zero babies.
- [x] Browser check deferred by design: there is no baby data to trigger it live.
