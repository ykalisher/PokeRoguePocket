...
Checks:
  1. starter picked            -> runs.started, 'first-steps' unlocked + pending
  ...
  6. battle won                -> battles.won + battles.won.rank.<Rank>,
                                  'first-blood' unlocked, reload-safe
...
def main():
    ...
    with serving() as base, sync_playwright() as p:
        ...
        profile = page.evaluate(PROFILE_PROBE)
        check(problems, "first-steps" in profile["unlocked"], "'first-steps' unlocked")
        check(problems, "first-steps" in profile["pendingUnlocks"],
              "'first-steps' queued in pendingUnlocks")
        ...
        profile = page.evaluate(PROFILE_PROBE)
        check(problems, "first-blood" in profile["unlocked"], "'first-blood' unlocked")
        check(problems, "first-blood" in profile["pendingUnlocks"],
              "'first-blood' queued in pendingUnlocks")
        ...
        check_stat(problems, page, "runs.completed", 1)
        check_stat(problems, page, "runs.completed.starter.water", 1)
        check(problems, "champion" in page.evaluate(PROFILE_PROBE)["unlocked"],
              "'champion' unlocked")
